...
mod tests {
    ...
    fn test_process_start_registers_subagent() {
        ...
        let changed = map.process_start(None, "code-reviewer", None);
        assert!(changed);
    ...
    fn test_process_stop_marks_oldest_working_of_same_type() {
        ...
        let changed = map.process_stop(None, "code-reviewer");
        assert!(changed);
    ...
    fn test_process_stop_fallback_to_any_working() {
        let mut map = SubagentMap::new();
        map.process_start(None, "code-reviewer", None);
        let changed = map.process_stop(None, "unknown-type");
        assert!(changed);
        assert_eq!(map.subagents()[0].status, SessionStatus::Finished);
    }
